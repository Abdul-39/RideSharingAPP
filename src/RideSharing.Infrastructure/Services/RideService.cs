...
public class RideService : IRideService
{
  ...
  public async Task<ApiResponse<RideDto>> CreateFromMatchAsync(Guid userId, Guid matchId)
  {
    ...
    _db.RideHistories.Add(new RideHistory
    {
      ...
      Note = "Ride created from accepted match",
  ...
  public async Task<ApiResponse<RideDto>> ConfirmAsync(Guid id, Guid userId)
  {
    ...
    var roleName = participant.Role.ToString(); // Driver | Passenger
    ...
    // Broadcast / inbox: other party must NOT see "Your confirmation..."
    var broadcastMsg = allConfirmed
        ? "Both parties confirmed. Ride is confirmed."
        : $"{roleName} confirmed the ride. Waiting for the other party to confirm.";

    // HTTP response for the person who clicked Confirm
    var actorMsg = allConfirmed
        ? "Both parties confirmed. Ride is confirmed."
        : "Your confirmation recorded. Waiting for the other party.";

    var dto = await LoadDto(id);
    await PublishStatusAsync(id, dto.Status, null, broadcastMsg, userId);
    return ApiResponse<RideDto>.SuccessResponse(dto, actorMsg);
  }
  ...
  private static RideDto Map(Ride r) => new()
  {
    ...
    ScheduledDepartureTime = r.ScheduledDepartureTime.ToString("HH:mm"),
