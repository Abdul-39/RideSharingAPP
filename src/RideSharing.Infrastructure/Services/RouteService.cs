...
public class RouteService : IRouteService
{
  ...
  public async Task<ApiResponse<List<RouteDto>>> GetMyRoutesAsync(Guid userId)
  {
    ...
    return ApiResponse<List<RouteDto>>
        .SuccessResponse(routes.Select(Map).ToList());
  }

  public async Task<ApiResponse<RouteDto>> GetByIdAsync(
      Guid id,
      Guid userId,
      bool isAdmin)
  {
    ...
    if (route == null)
      return ApiResponse<RouteDto>
          .FailureResponse("Route not found.");

    if (!isAdmin && route.UserId != userId)
      return ApiResponse<RouteDto>
          .FailureResponse("You are not allowed to view this route.");

    return ApiResponse<RouteDto>
        .SuccessResponse(Map(route));
  }

  public async Task<ApiResponse<RouteDto>> CreateAsync(
      Guid userId,
      CreateRouteRequest request)
  {
    ...
    {
      return ApiResponse<RouteDto>
          .FailureResponse("Invalid departure time.");
    }
    ...
    var route = new Route
    {
      ...
      MaximumTimeToleranceMinutes =
            request.MaximumTimeToleranceMinutes,
      ...
      IsDriverRoute = request.IsDriverRoute,

      AvailableSeats = request.IsDriverRoute
  ...
  public async Task<ApiResponse<RouteDto>> UpdateAsync(
      Guid id,
      Guid userId,
      bool isAdmin,
      UpdateRouteRequest request)
  {
    ...
    if (!isAdmin && route.UserId != userId)
    {
      return ApiResponse<RouteDto>
          .FailureResponse(
              "You are not allowed to modify this route.");
    }
    ...
    route.MaximumTimeToleranceMinutes =
        request.MaximumTimeToleranceMinutes;
    ...
    route.IsDriverRoute = request.IsDriverRoute;

    route.AvailableSeats = request.IsDriverRoute
        ? request.AvailableSeats
        : 0;
    ...
    catch (DbUpdateConcurrencyException)
    {
      ...
      return ApiResponse<RouteDto>
          .FailureResponse(
              "Route was modified by another process. Please retry.");
    }

    var reloaded = await _db.Routes
        .AsNoTracking()
        .Include(r =>
            r.RideSchedules.Where(s => !s.IsDeleted))
      ...
      bool isAdmin)
  {
    ...
    return ApiResponse
        .SuccessResponse("Route deleted.");
  }

  /// <summary>
  /// Calls OSRM and stores road points on the route
  /// for corridor matching.
  /// </summary>
  private async Task FillPolylineAsync(
      Route route,
      CancellationToken ct = default)
  {
    try
    {
      ...
      {
        points =
            LehtrarCorridorPlaces
                .EnrichWithNearbyPlaces(
                    points,
                    maxKm: 1.5)
                .ToList();

        route.RoutePolylineJson =
            RoutePolylineCodec.Serialize(points);
    ...
    {
      // Leave polyline null.
      // Matching can call OSRM live later.
    }
  }

  private static RouteDto Map(Route r)
  {
    return new RouteDto
    {
      Id = r.Id,
