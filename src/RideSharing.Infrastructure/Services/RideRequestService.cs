...
public class RideRequestService : IRideRequestService
{
  ...
  private readonly ILogger<RideRequestService> _log;

  public RideRequestService(
      RideSharingDbContext db,
      IMatchingService matching,
      INotificationService notifications,
      ILogger<RideRequestService> log)
  {
    _db = db;
    _matching = matching;
  ...
  public async Task<ApiResponse<RideRequestDto>> CreateAsync(
      Guid userId,
      CreateRideRequestDto request)
  {
    ...
    if (route == null)
    {
      return ApiResponse<RideRequestDto>.FailureResponse(
          "Route not found or not owned by you.");
    }
    ...
    {
      return ApiResponse<RideRequestDto>.FailureResponse(
          "Seats needed must be between 1 and 20.");
    }

    if (request.TimeToleranceMinutes < 0 ||
        request.TimeToleranceMinutes > 120)
    {
      return ApiResponse<RideRequestDto>.FailureResponse(
          "Time tolerance must be 0–120 minutes.");
    }
    ...
    // AUTO MATCHING
    // ============================================================
    // Matching automatically runs after the ride request is created.
    //
    // IMPORTANT:
    // Previously exceptions were silently ignored with:
    //
    // catch { }
    //
    // That made it impossible to know why matching was returning
    // "0 matches".
    //
    // Now we log the actual exception in Visual Studio Output/Console.
    // ============================================================

    try
    {
      _log.LogInformation(
          "Starting automatic matching for RideRequest {RequestId}. Route: {RouteId}, User: {UserId}",
          entity.Id,
          entity.RouteId,
          entity.UserId);
      ...
      _log.LogInformation(
          "Automatic matching completed for RideRequest {RequestId}. Matches found: {MatchCount}",
          entity.Id,
          matches?.Count ?? 0);
    }
    catch (Exception ex)
    {
      _log.LogError(
          ex,
          "MATCHING FAILED for RideRequest {RequestId}. Route: {RouteId}, User: {UserId}",
          entity.Id,
          entity.RouteId,
          entity.UserId);
    }

    return ApiResponse<RideRequestDto>.SuccessResponse(
        Map(entity),
        "Ride request created.");
  }

  public async Task<ApiResponse<List<RideRequestDto>>> GetMyAsync(
      Guid userId)
  {
    ...
    return ApiResponse<List<RideRequestDto>>.SuccessResponse(
        list.Select(Map).ToList());
  }

  public async Task<ApiResponse<RideRequestDto>> GetByIdAsync(
      Guid id,
      Guid userId,
      bool isAdmin)
  {
  ...
  public async Task<ApiResponse> CancelAsync(
      Guid id,
      Guid userId)
  {
    var entity = await _db.RideRequests
  ...
  public async Task<ApiResponse<List<MatchResultDto>>> RunMatchingAsync(
      Guid requestId,
      Guid userId)
  {
    var entity = await _db.RideRequests
    ...
    if (entity == null)
    {
      return ApiResponse<List<MatchResultDto>>.FailureResponse(
          "Request not found.");
    }
    ...
    foreach (var m in matches)
    {
      ...
      catch (Exception ex)
      {
        _log.LogWarning(
            ex,
            "Failed to send new-match notification for Match {MatchId}",
            m.MatchId);
      }
    }

    return ApiResponse<List<MatchResultDto>>.SuccessResponse(
        matches.ToList(),
        matches.Count == 0
            ? "No matches found."
  ...
  public async Task<ApiResponse<List<MatchResultDto>>> GetMatchesAsync(
      Guid requestId,
      Guid userId)
  {
    var entity = await _db.RideRequests
    ...
    var dtos = matches.Select(m => new MatchResultDto
    {
      ...
      MatchedUserName =
            m.MatchedUser != null
                ? $"{m.MatchedUser.FirstName} {m.MatchedUser.LastName[0]}."
                : "",

      IsVerified =
            m.MatchedUser?.IsVerified ?? false,

      Gender =
            m.MatchedUser?.Gender.ToString() ?? "",
      ...
      SeatingCapacity =
            m.Vehicle?.SeatingCapacity,

      MatchedRouteSource =
            m.MatchedRoute?.SourceAddress,

      MatchedRouteDestination =
            m.MatchedRoute?.DestinationAddress,

      MatchedDepartureTime =
            m.MatchedRoute?.PreferredDepartureTime
                .ToString("HH:mm"),
    ...
    return ApiResponse<List<MatchResultDto>>
        .SuccessResponse(dtos);
  }

  public async Task<ApiResponse> RespondToMatchAsync(
      Guid matchId,
      Guid userId,
      bool accept)
  {
    ...
    if (accept)
    {
      match.RideRequest.Status =
          RideRequestStatus.Matched;

      var others = await _db.RideMatches
          .Where(m =>
              m.RideRequestId == match.RideRequestId &&
              m.Id != matchId &&
      ...
      if (!await _db.Rides.AnyAsync(r =>
              r.RideMatchId == matchId &&
              !r.IsDeleted))
      {
        ...
        _db.Rides.Add(new Ride
        {
          ...
          ScheduledDepartureTime =
                match.RideRequest.PreferredDepartureTime,
          Status = RideStatus.Matched,
          CreatedAt = DateTime.UtcNow
        });

        _db.RideParticipants.Add(
            new RideParticipant
            {
              Id = Guid.NewGuid(),
              RideId = rideId,
    ...
    if (accept)
    {
      ...
      catch (Exception ex)
      {
        _log.LogWarning(
            ex,
            "Failed to send match-accepted notification for Match {MatchId}",
            matchId);
      }
    }
    ...
    return ApiResponse.SuccessResponse(
        accept
            ? "Match accepted. Driver notified."
            : "Match rejected.");
  }
  ...
  private static RideRequestDto Map(RideRequest r) => new()
  {
    ...
    SourceLatitude =
          r.Route?.SourceLatitude ?? 0,

    SourceLongitude =
          r.Route?.SourceLongitude ?? 0,

    DestinationLatitude =
          r.Route?.DestinationLatitude ?? 0,

    DestinationLongitude =
          r.Route?.DestinationLongitude ?? 0,

    TravelDate =
          r.TravelDate,

    PreferredDepartureTime =
          r.PreferredDepartureTime.ToString("HH:mm"),

    SeatsNeeded =
          r.SeatsNeeded,

    GenderPreference =
          r.GenderPreference.ToString(),

    TimeToleranceMinutes =
          r.TimeToleranceMinutes,
    ...
    MatchCount =
          r.Matches?.Count(m => !m.IsDeleted) ?? 0
  };
}
