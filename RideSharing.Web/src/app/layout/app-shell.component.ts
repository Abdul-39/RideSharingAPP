...
@Component({
        ...
        <div class="bar">
          ...
          <div class="bar-right">
            ...
            @if (hubState() === 'connected') {
              <span class="live-dot" title="Live connected">● Live</span>
            }
  ...
  styles: [`
    ...
    .live-dot { font-size: 0.68rem; font-weight: 800; color: #0d9f6e; }
    ...
    .btn-icon { position: relative; width: 38px; height: 38px; border-radius: 12px; display: grid; place-items: center; background: #fff; border: 1px solid #e2e8f0; color: #334155; text-decoration: none; box-shadow: 0 1px 3px rgba(15,23,42,0.04); flex-shrink: 0; }
    .btn-icon:hover { border-color: #b7ebc9; color: #0d9f6e; background: #f0fdf6; }
    .badge { position: absolute; top: -4px; right: -4px; min-width: 18px; height: 18px; padding: 0 4px; border-radius: 999px; background: #e11d48; color: #fff; font-size: 0.65rem; font-weight: 800; display: grid; place-items: center; border: 2px solid #fff; }
    ...
    .toast span { font-size: 0.78rem; opacity: 0.9; }
...
export class AppShellComponent implements OnInit, OnDestroy {
  ...
  refreshUnread(): void {
    this.http.get<any>(`${this.api}/notifications/unread-count`).subscribe({
      next: (r) => {
        const n = r?.data?.count ?? r?.data ?? r?.count ?? 0;
      ...
      error: () => {
        ...
        this.http.get<any>(`${this.api}/notifications`).subscribe({
          next: (r) => {
            ...
            this.unread.set(arr.filter((x: any) => !x.isRead).length);
          },
          error: () => {}
        });
      }
